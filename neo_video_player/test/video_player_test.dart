...
class FakeController extends ValueNotifier<NeoVideoPlayerValue>
    implements NeoVideoPlayerController {
  FakeController() : super(NeoVideoPlayerValue(duration: Duration.zero));
  ...
  @override
  int textureId = NeoVideoPlayerController.kUninitializedTextureId;
  ...
  Future<void> togglePip() {
    // TODO(matosa): implement togglePip
...
void main() {
  ...
  void verifyPlayStateRespondsToLifecycle(
    NeoVideoPlayerController controller, {
  ...
  testWidgets('update texture', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(NeoVideoPlayer(controller));
  ...
  testWidgets('update controller', (WidgetTester tester) async {
    final controller1 = FakeController()..textureId = 101;
    await tester.pumpWidget(NeoVideoPlayer(controller1));
    expect(
    ...
    final controller2 = FakeController()..textureId = 102;
    await tester.pumpWidget(NeoVideoPlayer(controller2));
    expect(
      find.byWidgetPredicate(
  ...
  testWidgets('non-zero rotationCorrection value is used',
    ...
    final controller = FakeController.value(
      NeoVideoPlayerValue(duration: Duration.zero, rotationCorrection: 180),
    )..textureId = 1;
    await tester.pumpWidget(NeoVideoPlayer(controller));
    final actualRotationCorrection =
        find.byType(Transform).evaluate().single.widget as Transform;
    final actualRotationCorrectionStorage =
        actualRotationCorrection.transform.storage;
    final expectedMatrixStorage = Matrix4.rotationZ(math.pi).storage;
    expect(
      actualRotationCorrectionStorage.length,
      equals(expectedMatrixStorage.length),
    );
    for (var i = 0; i < actualRotationCorrectionStorage.length; i++) {
      expect(
  ...
  testWidgets('no transform when rotationCorrection is zero',
      (WidgetTester tester) async {
    final controller =
        FakeController.value(NeoVideoPlayerValue(duration: Duration.zero))
          ..textureId = 1;
    await tester.pumpWidget(NeoVideoPlayer(controller));
  ...
  group('ClosedCaption widget', () {
    ...
    testWidgets(
      'Passes text contrast ratio guidelines',
      (WidgetTester tester) async {
        const text = 'foo';
        await tester.pumpWidget(
          const MaterialApp(
  ...
  group('VideoPlayerController', () {
    group('initialize', () {
      ...
      test('asset', () async {
        final controller = NeoVideoPlayerController.asset(
          'a.avi',
        );
        await controller.initialize();
      ...
      test(
        'file',
        () async {
          final controller = NeoVideoPlayerController.file(File('a.avi'));
          await controller.initialize();

          final uri = fakeVideoPlayerPlatform.dataSources[0].uri!;
          expect(
            uri.startsWith('file:///'),
            true,
      ...
      test(
        ...
        () async {
          final controller =
              NeoVideoPlayerController.file(File('A #1 Hit.avi'));
          await controller.initialize();
      ...
      test(
        ...
        () async {
          final controller = NeoVideoPlayerController.file(
            File('a.avi'),
            httpHeaders: <String, String>{'Authorization': 'Bearer token'},
          );
          await controller.initialize();
    ...
    test('contentUri', () async {
      final controller =
          NeoVideoPlayerController.contentUri(Uri.parse('content://video'));
      await controller.initialize();
    ...
    test('dispose', () async {
      ...
      expect(
        ...
        NeoVideoPlayerController.kUninitializedTextureId,
      );
    ...
    group('scrubbing', () {
      testWidgets('restarts on release if already playing',
        ...
        final progressRect = tester.getRect(find.byWidget(progressWidget));
        await tester.dragFrom(progressRect.center, const Offset(1, 0));
    ...
    group('Platform callbacks', () {
      ...
      testWidgets('buffering status', (WidgetTester tester) async {
        ...
        const bufferStart = Duration.zero;
  ...
  group('VideoPlayerValue', () {
    test('uninitialized()', () {
      final uninitialized = NeoVideoPlayerValue.uninitialized();
    ...
    test('erroneous()', () {
      ...
      final error = NeoVideoPlayerValue.erroneous(errorMessage);
    ...
    test('toString()', () {
      ...
      final value = NeoVideoPlayerValue(
        duration: duration,
        size: size,
        position: position,
        caption: caption,
      ...
      expect(
          value.toString(),
          'NeoVideoPlayerValue(duration: 0:00:05.000000, '
    ...
    group('copyWith()', () {
      test('exact copy', () {
        final original = NeoVideoPlayerValue.uninitialized();
        final exactCopy = original.copyWith();

        expect(exactCopy.toString(), original.toString());
      });
      test('errorDescription is not persisted when copy with null', () {
        final original = NeoVideoPlayerValue.erroneous('error');
        final copy = original.copyWith(errorDescription: null);

        expect(copy.errorDescription, null);
      });
      test('errorDescription is changed when copy with another error', () {
        final original = NeoVideoPlayerValue.erroneous('error');
        final copy = original.copyWith(errorDescription: 'new error');
